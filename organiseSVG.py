...
class svg(object):
    ...
    def getWidth(self):
        """Gets width of svg from viewbox."""
    ...
    def getHeight(self):
        """Gets height of svg from viewbox."""
        internalSvg = self.svg.xpath('/svg:svg/svg:g/svg:svg',\
                namespaces={'svg': 'http://www.w3.org/2000/svg'})
        if len(internalSvg) == 1 and type(internalSvg) == list:
            viewBox = internalSvg[0].xpath('./@viewBox')
...
class rect(object):
    """Basic class for the packing algorithm.
    
    self.widths and self.heights are lists containing the widths and 
    heights of an irregular grid of cells. 
    
    self.occMat is the occupation matrix. True indicates that a cell is
    occupied, False indicates that it is not."""
    ...
    def findVacant(self,width,height):
        """Find a vacant spot of the given dimensions."""
    ...
    def getBoundingRect(self):
        """Gets smallest rectangle that contains all of the svgs using self.occMat."""
    ...
    def split(self,vacantSpot,width,height):
        ...
        Used When an svg is placed in a cell that is too large for it (i.e. most
        of the time.)"""
    ...
    def occupy(self,vacantSpot):
        """Occupies a range of cells.
        
        vacantSpot is a dict with keys 'leftIdx', 'rightIdx', 'topIdx', 
        'bottomIdx'"""
...
class packer(object):
    """Packs a list of svgs to take up a reasonably small amount of space.
    
    Implements sprite packing."""
    def __init__(self,svgStringList):
        ...
        self.bestWidth = self.currentWidth + 100
        self.bestHeight = self.currentHeight + 100
    ...
    def makeSVG(self,bestOrCurrent,withRect):
        """Makes the svg after the packing algorithm is done."""

        svgString = '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:cml="http://www.xml-cml.org/schema"  x="0" y="0" width="200px"  height="200px" viewBox="0 0 100 100" font-family="sans-serif" stroke="rgb(0,0,0)" stroke-width="1"  stroke-linecap="round"></svg>'
        ...
        innerSvg.set('width','100')
        innerSvg.set('height','100')
        innerSvg.set('x','0')
        innerSvg.set('y','0')
